...
def _run_pipeline(job_id: str, sources: list[SourceInput], documents: list[DocumentInput]) -> None:
    ...
    try:
        ...
        codebase_models: list[CodebaseModel] = _run_stage_map(
            sources, code_ingester.ingest, non_fatal
        )

        document_models: list[DocumentModel] = _run_stage_map(
            documents, document_ingester.ingest, non_fatal
        )
        ...
        code_analyses: list[AnalysisResult] = _run_stage_map(
            codebase_models, ai.analyze_code, non_fatal
        )

        doc_analyses: list[AnalysisResult] = _run_stage_map(
            document_models, ai.analyze_document, non_fatal
        )
        ...
        # Reconcile — fallback to a minimal model on failure
        fallback_reconciled = ReconciledModel(
            elements=[],
            relationships=[],
        ...
        reconciled: ReconciledModel = _run_stage(
            lambda: ai.reconcile(merged_code, merged_doc),
            non_fatal,
            default=fallback_reconciled,
        ) or fallback_reconciled

        # ---- Stage 3: Generation ----
        job.stage = JobStage.generating

        # LeanIX mapping
        def _leanix_stage() -> None:
            leanix_report = leanix_mapper.map(reconciled.elements)
            job.results["leanix_report"] = leanix_report.model_dump()

        _run_stage(_leanix_stage, non_fatal)
        ...
        diagram_results = _run_stage_map(
            diagram_generators,
            lambda pair: pair[1](reconciled),
            non_fatal,
        )
        # Post-process successful diagram outputs
        diagram_outputs = []
        for diag in diagram_results:
        ...
        _run_stage(_adr_stage, non_fatal)

        # Markdown documentation
        def _markdown_stage() -> None:
        ...
        _run_stage(_markdown_stage, non_fatal)

        # ---- Stage 4: Serialization ----
        job.stage = JobStage.serializing

        def _serialization_stage() -> None:
        ...
        _run_stage(_serialization_stage, non_fatal)
